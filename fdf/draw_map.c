...
/*   Updated: 2021/08/23 18:18:49 by cflorind         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int	point_in_screen(t_fdf *vars, t_point offset,
	int ix, int iy)
{
	if (vars->map[iy][ix][X] * vars->map_scale + offset.x
		<= vars->screen_size_x
		&& vars->map[iy][ix][X] * vars->map_scale + offset.x >= 0
		&& vars->map[iy][ix][Y] * vars->map_scale + offset.y
			<= vars->screen_size_y
		&& vars->map[iy][ix][Y] * vars->map_scale + offset.y >=0)
		return (1);
	return (0);
}
...
static inline void	draw_line_x(t_fdf *vars, t_point offset, t_point i)
{
	...
	{
		end.x = vars->map[i.y][i.x][X] * vars->map_scale + offset.x;
		end.y = vars->map[i.y][i.x][Y] * vars->map_scale + offset.y;
