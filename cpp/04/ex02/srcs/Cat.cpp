...
/*   Updated: 2022/03/21 14:27:45 by cflorind         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Cat::Cat(std::string name): AAnimal("Cat"), brain(new Brain())
{
	this->name = name;
	//this->brain = new Brain();
...
Cat::Cat(Cat const &inst): AAnimal("Cat")
{
	*this = inst;
...
{
	std::cout << this->getType() << " " << getName() << " Myau" << std::endl;
}
